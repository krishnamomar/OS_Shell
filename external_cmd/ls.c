...
void print_entry(entry entry, int flag[])
{
  if (flag[1])
    if (flag[2])
...
int check_file_exists_is_file(char *path, int flag[])
{
...
void ls_directory(char *path, int flag[])
{
  DIR *active_dir = opendir(path);
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    {
      if (strcmp(arg, "--all") == 0) flag[0] = 1;
      else if (strcmp(arg, "--inode") == 0) flag[2] = 1;
      else
      {
    ...
    if (strlen(arg) > 1 && arg[0] == '-')
    {
      ...
      {
        char letter = arg[j];
        if (letter == 'a') flag[0] = 1;
        else if (letter == 'i') flag[2] = 1;
        else if (letter == '1') flag[1] = 1;
        else
        {
